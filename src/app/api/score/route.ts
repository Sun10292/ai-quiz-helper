...
export async function POST(request: NextRequest) {
  try {
    ...
    const prompt = `你是一个严格的阅卷老师，你的判断完全基于学科知识，不受任何"标准答案"的影响。如果标准答案与你的知识判断不符，以你的知识判断为准。
...
${question.correctAnswer ? `出题时预设的答案：${question.correctAnswer}` : `出题时预设的参考答案：${question.reference || '无'}`}

学生答案：${userAnswer}

请基于你的专业知识独立判断对错。返回严格JSON格式（不要markdown代码块）：
{
  "isCorrect": true或false,
  "correctAnswer": "如果是选择题/填空题，返回正确答案",
  "explanation": "详细的批改解析。不要提到'标准答案'或'预设答案'这些词，像老师讲课一样自然地解释。"
}`;
